...
#include "libdac_internal.h"
    ...
    /**
     * @fn  int compute_secret_K(unsigned char *secret_K, const unsigned char *s_secret, const unsigned char *c_public)
     *
     * @brief   Function that computes shared secret from Diffie-Hellman key exchange
     *
     * @param   secret_K    Shared secret
     * @param   s_secret    Own private key
     * @param   c_public    Received public key
     *
     * @return  0 if it succeeds, 1 if it fails.
     */
int compute_secret_K(unsigned char *secret_K, const unsigned char *s_secret, const unsigned char *c_public);
...
int generate_enc_auth_keys(unsigned char *hash, unsigned char *shared_secret_K, unsigned char *shared_H, char magic_letter);
    ...
    /**
     * @fn  void print_key(unsigned char *buffer, int length);
     *
     * @brief   Function that prints key content in console
     *
     * @param   buffer      Buffer containg the key
     * @param   length      Key length
     *
     */
void print_key(unsigned char *buffer, int length);
